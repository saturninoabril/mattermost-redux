...
function combineUserActivitySystemMessage(newPost, firstPost) {
    ...
    let userActivityIds = firstPost.user_activity_ids;
    message += ` ${newPost.message}`;
    userActivityIds = [...userActivityIds, newPost.id];
    ...
    return {
        ...firstPost,
        message,
        user_activity_ids: userActivityIds,
        props: newProps
    };
}
...
function combineUserActivitySystemMessages(posts = {}, order = []) {
    ...
    return order.reduce((acc, postId, index, arr) => {
        ...
        if (combinedPosts && (arr.length === index + 1 || combinedPosts.user_activity_ids.length === POST_PROPS_USER_ACTIVITIES_MAX)) {
            combinedPosts = pushCombinedPosts(combinedPosts);
        }
